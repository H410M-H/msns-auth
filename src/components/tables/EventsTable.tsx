...
export default function EventsTable({ onEventView, onEventEdit }: EventsTableProps) {
  ...
  const { data, isLoading, refetch } = api.event.getAll.useQuery({
    ...
    status: statusFilter !== "all" ? statusFilter as "CONFIRMED" | "TENTATIVE" | "CANCELLED" : undefined,
    offset,
    limit,
  })
  ...
  const filteredAndSortedEvents = useMemo(() => {
    ...
    return filtered
  }, [events, typeFilter, priorityFilter, statusFilter, sortField, sortDirection])
      ...
      <CardContent className="space-y-4">
                    ...
                    <TableCell className="font-medium">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          {event.description && (
                            <div className="text-sm text-gray-400 truncate mt-1">{event.description}</div>
                          )}
                        </div>
                      </div>
                    </TableCell>
        ...
        {totalEvents > limit && (
          <div className="flex justify-between items-center text-sm text-gray-400 pt-4">
            ...
            <span>
              Page {Math.floor(offset / limit) + 1} of {Math.ceil(totalEvents / limit)}
            </span>
            <Button
              variant="outline"
              size="sm"
